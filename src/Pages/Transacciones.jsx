...
const Transacciones = () => {
    ...
    async function getAlltransactions() {
       try {
        const result = await fetch(`${import.meta.env.VITE_API_URL}/transacciones`,{
            method:'GET',
            credentials:'include'
        })
    ...
    function sortTransactionsBydate(params) {
      ...
      const sortedTransactions = updatedArray.sort((a,b) => new Date(a.created_at) - new Date(b.created_at) )
  ...
  return (
   <>
    ...
    <div className="mx-auto w-ful md:w-[70%] mt-8 flex flex-row-reverse justify-center items-center gap-20">
        <p className=" text-right font-bold text-lg">Total: <span>{formatearDinero(total)}</span></p>
        <button
        onClick={handleExport}
        className="btn btn-outline btn-accent">Exportar a Excel</button>
    </div>
   </>
  )
}
